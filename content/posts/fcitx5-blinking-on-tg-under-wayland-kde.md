...
description: 如果你在使用 fcitx5 输入法时，遇到 Telegram 等客户端中文字候选框闪烁、跳动的问题，这篇文章或许能帮你解决。本文详细记录了在 Fedora 36 KDE Wayland 环境下，通过调整输入法前端设置、清理错误环境变量等方式，彻底解决 fcitx5 候选框闪烁问题的全过程。内容包含具体操作步骤、可能遇到的意外情况及其排查方法，还介绍了如何通过 dbus 命令和 /proc 文件系统检查输入法状态和环境变量。适合使用 Linux 桌面、尤其是 KDE 和 fcitx5 的用户阅读。
sticky:
tags:
- Linux
- 笔记
